...
class App extends React.Component {
  ...
  initSocket(url) {
    ...
    socket.on('logout', (data) => {
      this.appendMessage(`${data.username} déconnecté.`);
      this.setState({users: data.users});
    });
    this.socket = socket;
  }
  ...
  render() {
    ...
    return (
      <aside>
        ...
        <div id="user-stats">{users.length} utilisateurs connectés.</div>
      </aside>
    );
  }
}
    ...
    return (
      <div className="input">
        <input
          ...
          id="text-input"
          ref="input"
          placeholder="git quelque chose..."
          onInput={this.onInput}
          onKeyDown={this.onKeyDown}
        />
        <button id="send-btn" onClick={this.onClick}>Envoyer</button>
      </div>
    );
  }
}
